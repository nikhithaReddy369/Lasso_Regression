...
```{r work-distribution, fig.cap="Figure 1: Work Experience Distribution in 1976", label="fig:work-distribution", fig.width=10, fig.height=5}
library(ggplot2)
...
```{r wage-distribution, fig.cap="Figure 2: Wage Distribution in 1976",fig.width=10, fig.height=5,label="fig:wage-distribution"}
...
p_wage
```
The histogram and density plot for `wage76` reveal the distribution of wages among workers in 1976, showing a typical pattern that indicates economic inequalities [Figure 2](#fig:wage-distribution):
...
```{r correlation-matrix, fig.cap="Figure 3: Correlation Matrix of Selected Variables",fig.width=10, fig.height=5,label="fig:correlation-matrix"}
library(corrplot)
...
corrplot(cor_matrix, method = "color", 
         ...
         mar = c(0, 0, 2, 0))
```
The correlation matrix is a powerful tool used to visualize and quantify the relationships between key variables such as `wage76`, `grade76`, `exp76`, and `age76` in the `RetSchool` dataset. Here’s what we learn from it [Figure 3](#fig:wage-distribution):
...
Understanding how these variables interact is easier when we visualize their distributions and relationships. [Figure 4](#fig:visualizations-key-variables)

```{r visualizations-key-variables, fig.cap="Figure 4:Visualizations of Key Variables", fig.width=10, fig.height=5, lable="fig:visualizations-key-variables"}
library(ggplot2)
...
library(patchwork)
p_wage + p_education + p_experience + plot_layout(ncol = 1)
```
...
The regularization parameter ($\lambda$) in LASSO regression critically influences model accuracy by balancing model complexity and predictive performance. Selecting the optimal ($\lambda$) is essential to minimize overfitting and ensure the model generalizes well [Figure 6](#fig:cross-validation-visualization).

```{r cross-validation-visualization, echo=TRUE, fig.cap="Figure 6: Cross-Validation Curve",fig.width=10, fig.height=5,lable="fig:cross-validation-visualization"}
library(glmnet)
library(ggplot2)
library(dplyr)
...
[Figure 7](#fig:optimal-coefficients-visualization) visualizes these coefficients, highlighting the relative importance of each predictor in the context of wage determination.


```{r optimal-coefficients-visualization,lable="fig:optimal-coefficients-visualization", echo=TRUE, message=FALSE, warning=FALSE, fig.cap="Figure 7: Visualization of LASSO Coefficients",fig.width=10, fig.height=5}
library(knitr)
library(glmnet)
...
```{r visualization,lable="fig:visualization", echo=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Figure 8: Comparison of Coefficients from MLR and LASSO",fig.width=10, fig.height=7.5}
library(ggplot2)
library(dplyr)
...
The chart below, titled "[Figure 8: Comparison of Coefficients from MLR and LASSO](#fig:visualization)," illustrates how LASSO's regularization alters the coefficients:
...
```{r continuous-impact, fig.cap="Figure 9:Impact of Continuous Variables on Wages",fig.width=10, fig.height=5,lable="fig:continuous-impact"}
...
```
These visual aids [Figure 9](#fig:continuous-impact) are crucial for demonstrating how incremental changes in age and education relate to changes in wages, providing a compelling argument for the continuous benefits of education.
