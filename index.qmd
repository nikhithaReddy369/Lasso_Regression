...
3. Subgroup Analysis
4. Interaction Terms
5. Dropping rows with any missing data
6. Logistic Regression with LASSO for Categorical Outcome Prediction

#### 1. LASSO Regression for Composite Score Prediction

Model Used: LASSO regression was employed to predict a composite score created from standardized wage76, grade76, and exp76 variables.

Rationale: LASSO (Least Absolute Shrinkage and Selection Operator) regression was chosen for its ability to perform variable selection and regularization. By penalizing the absolute size of the regression coefficients, LASSO helps in reducing overfitting and identifying the most significant predictors.
...
```
Results: The optimal lambda value identified through cross-validation minimized the mean squared error (MSE) and improved the model's predictive accuracy. The model explained approximately 42.4% of the variance in the composite score, indicating a moderate level of predictive accuracy.

#### 2. Enhanced Cross-Validation in LASSO Regression
Model Used: LASSO regression, tuned through K-fold cross-validation.

Rationale: The LASSO regression model was utilized for its ability to perform both variable selection and regularization. The inclusion of K-fold cross-validation serves to identify the optimal lambda (λ) value that balances model complexity and accuracy, effectively addressing overfitting concerns and enhancing model reliability.

Cross-Validation Process: K-fold cross-validation systematically divides the dataset into K subsets, or "folds." The model is trained on K-1 folds and validated on the remaining fold, iterating through all K combinations. This process allows for the evaluation of the model's performance across diverse subsets of the data, offering a comprehensive assessment of its predictive capabilities.

Optimal Lambda Selection: The lambda parameter controls the degree of LASSO's penalty on the model's coefficients, with a higher lambda enforcing greater sparsity (more coefficients set to zero). The cross-validation process seeks to find the lambda value that minimizes prediction error, striking a balance between simplicity and predictive power.
...
Results: The LASSO regression model, after cross-validation, demonstrated an MSE of 0.1575524 and an R² value of 0.4173462. These metrics signify the model's moderate success in capturing the relationship between the predictors (standardized wage76, grade76, exp76) and the composite outcome. The optimal lambda value identified was 0.0007330395, indicating an effective balance was found.



#### 3. Subgroup Analysis by Racial Background

Model Used: Separate LASSO regressions were conducted for individuals identified as black and non-black.

Rationale: Subgroup analysis allowed for the exploration of the model's performance across different demographic groups, potentially uncovering distinct patterns and predictors of importance within each subgroup.
...
Results: The model demonstrated different levels of predictive accuracy across subgroups, with the non-black subgroup showing a better fit. This highlights the importance of considering demographic factors in predictive models.


#### 4. Incorporation of Interaction Terms
Model Used: LASSO regression with interaction terms.

Rationale: Interaction terms were included to capture the combined effects of pairs of predictors, providing insights into the non-additive relationships between variables.
...
```
Results: The inclusion of interaction terms significantly improved the model's explanatory power, as evidenced by an increased R² value. This underscores the complexity of the relationships between predictors.



#### 5. Approach to Missing Data: Dropping Rows

Model Used: LASSO regression on a dataset with dropped rows containing any missing data.

Rationale: Dropping rows with missing data was employed as a straightforward approach to ensure the integrity of the model input.



```{r}
data_drop_na <- na.omit(data)

# Standardizing the variables 'wage76', 'grade76', and 'exp76'
preProcess_scale <- preProcess(data_drop_na[,c('wage76', 'grade76', 'exp76')], method = 'scale')
data_drop_na[,c('wage76', 'grade76', 'exp76')] <- predict(preProcess_scale, data_drop_na[,c('wage76', 'grade76', 'exp76')])

# Creating a composite score
data_drop_na$composite_score <- rowMeans(data_drop_na[,c('wage76', 'grade76', 'exp76')])

# Preparing the dataset
X_drop_na <- data_drop_na %>% select(-c('wage76', 'grade76', 'exp76', 'rownames', 'composite_score'))
y_drop_na <- data_drop_na$composite_score

# Splitting the data
set.seed(0)
trainIndex <- createDataPartition(y_drop_na, p = .8, 
                                  list = FALSE, 
                                  times = 1)
X_train_drop_na <- X_drop_na[trainIndex, ]
X_test_drop_na <- X_drop_na[-trainIndex, ]
y_train_drop_na <- y_drop_na[trainIndex]
y_test_drop_na <- y_drop_na[-trainIndex]

# No imputation needed here

# Fitting LASSO regression
# Note: LASSO requires matrix inputs, and 'y' should be a vector
model_lasso_drop_na <- cv.glmnet(as.matrix(X_train_drop_na), y_train_drop_na, alpha = 1)
y_pred_drop_na <- predict(model_lasso_drop_na, s = "lambda.min", newx = as.matrix(X_test_drop_na))

# Calculating MSE and R^2
mse_drop_na <- mean((y_test_drop_na - y_pred_drop_na)^2)
r2_drop_na <- cor(y_test_drop_na, y_pred_drop_na)^2

# Results for dropping rows with missing data
list(mse_drop_na, r2_drop_na)
```
Results: While this approach may reduce the available data, it simplifies the analysis and ensures that the model is trained on complete cases only.



#### 6. Logistic Regression with LASSO for Categorical Outcome

Model Used: Logistic regression with LASSO regularization was applied to predict a binary outcome derived from wage76.

Rationale: This model was selected for its ability to handle binary outcomes and perform feature selection simultaneously, identifying key predictors of wage being above or below the median.



```{r}
data <- readr::read_csv("RetSchool.csv", show_col_types = FALSE)
data <- na.omit(data)
library(caret)
library(glmnet)
library(pROC)

# Imputing missing values and creating a binary outcome based on median value
median_wage <- median(data$wage76, na.rm = TRUE)
data$wage76_imputed <- ifelse(is.na(data$wage76), median_wage, data$wage76)
data$wage76_binary <- ifelse(data$wage76_imputed > median_wage, 1, 0)

# Preparing the dataset for logistic regression
X_wage_binary <- data %>%
  select(-wage76, -wage76_imputed, -rownames, -wage76_binary) %>%
  as.data.frame()
y_wage_binary <- data$wage76_binary

# Splitting the data
set.seed(0)
list <- createDataPartition(y_wage_binary, p = 0.8, list = TRUE)
trainIndex <- list[[1]]
X_train_wage_binary <- X_wage_binary[trainIndex, ]
X_test_wage_binary <- X_wage_binary[-trainIndex, ]
y_train_wage_binary <- y_wage_binary[trainIndex]
y_test_wage_binary <- y_wage_binary[-trainIndex]

# Preprocessing the data (example, standardize)
preProcess_model <- preProcess(X_train_wage_binary, method = c("center", "scale"))
X_train_preprocessed_wage_binary <- predict(preProcess_model, X_train_wage_binary)
X_test_preprocessed_wage_binary <- predict(preProcess_model, X_test_wage_binary)

# Logistic regression with LASSO (L1 regularization)
X_train_matrix <- model.matrix(~ ., data = X_train_preprocessed_wage_binary)[, -1] # Removing intercept column
X_test_matrix <- model.matrix(~ ., data = X_test_preprocessed_wage_binary)[, -1]

cv_fit <- cv.glmnet(X_train_matrix, y_train_wage_binary, alpha = 1, family = "binomial")

# Predicting on the test set
y_pred_probs <- predict(cv_fit, newx = X_test_matrix, type = "response", s = "lambda.min")
y_pred_wage_binary <- ifelse(y_pred_probs > 0.5, 1, 0)

# Evaluating the model
conf_matrix <- table(y_test_wage_binary, y_pred_wage_binary)
roc_curve <- roc(y_test_wage_binary, y_pred_probs[,1])
...
```
Results: The AUC (Area Under the Curve) from the ROC (Receiver Operating Characteristic) analysis provided a measure of the model's ability to distinguish between the two wage categories, showcasing its predictive performance.


### Results and Analysis Summary:
### 1. LASSO Regression for Composite Score Prediction
#### Methodology and Results:
In this section, LASSO Regression was utilized to predict a composite score comprising wages, education, and experience. The model accounted for approximately 42.4% of the variance in the composite score, indicating a moderate level of effectiveness. The key to this model's performance was the optimal selection of the regularization parameter, λ, which balances model complexity against the risk of overfitting.

#### Analysis:
The effectiveness of LASSO Regression in this context underscores its utility in feature selection and complexity reduction. By penalizing the magnitude of coefficients, LASSO helps in identifying the most significant predictors while minimizing overfitting. This result is particularly relevant when dealing with high-dimensional data where multicollinearity might be an issue. The 42.4% variance explained suggests that while the model has moderate predictive power, there remains room for improvement, possibly through the exploration of additional predictors or alternative modeling techniques.

### 2. Enhanced Cross-Validation in LASSO Regression
####  Methodology and Results:
The application of K-fold cross-validation refined the selection of λ, leading to a model with an MSE of 0.1575524 and an R² of 0.4173462. This process enhances the model's reliability and robustness by ensuring its performance across various data subsets.

####  Analysis:
Cross-validation is crucial in guarding against the over-optimistic interpretation of the model's predictive ability on a single data set. By systematically applying it, this study ensured that the λ parameter was not just optimal for a specific dataset but generalized well across different subsets. The reported MSE and R² values indicate a model of moderate predictive accuracy, reinforcing the importance of cross-validation in establishing the reliability of predictive models.

### 3. Subgroup Analysis by Racial Background
####  Methodology and Results:
This analysis segment explored model performance across different racial subgroups, finding that non-black individuals exhibited a better model fit.

####  Analysis:
The variation in model performance by racial background emphasizes the influence of demographic factors on predictive models. This finding is critical for two reasons: firstly, it highlights potential disparities in the factors influencing wages, education, and experience across racial lines; secondly, it suggests that models which do not account for such subgroup differences might miss important nuances, leading to overgeneralized conclusions. The insight calls for the incorporation of demographic variables and possibly the development of tailored models for different subgroups to achieve better predictive accuracy and fairness.

### 4. Incorporation of Interaction Terms
####  Methodology and Results:
Including interaction terms between predictors significantly enhanced the model's explanatory power, as evidenced by an increased R² value.

#### Analysis:
The improvement in model performance with the addition of interaction terms underlines the complex interdependencies among the predictors. This complexity is often overlooked in simpler models that assume independent effects of predictors on the outcome. Interaction terms allow for the modeling of these effects more realistically, providing deeper insights into how different factors might amplify or diminish each other's impacts. This finding is particularly relevant in fields where the interaction between variables is known to influence outcomes significantly, such as in economics and psychology.

### 5. Approach to Missing Data - Dropping Rows
#### Methodology and Results:
The decision to handle missing data by excluding incomplete cases was aimed at preserving the integrity of the model input, at the potential cost of reducing the dataset size.

#### Analysis:
While dropping rows with missing data is a straightforward approach to ensure model reliability, it may lead to biased results if the missingness is not completely random. This method maintains simplicity and model integrity but might limit the analysis to a narrower dataset that may not be representative of the larger population. Future methodologies might consider imputation techniques to retain more data points without compromising the model's accuracy.

### 6. Logistic Regression with LASSO for Categorical Outcome Prediction
#### Methodology and Results:
This segment employed logistic regression with LASSO to predict categorical wage outcomes, with the model's effectiveness quantified by the AUC value.

#### Analysis:
Logistic regression with LASSO combines the benefits of logistic regression for binary outcome prediction and LASSO's feature selection capabilities. The AUC value serves as a robust measure of the model's ability to distinguish between categories, in this case, wage levels. This model's strength lies in its feature selection, which is crucial for identifying key predictors among potentially many correlated variables in wage classification.

### Conclusion
The detailed analysis underscores the multifaceted approach taken in this study, employing LASSO regression and logistic regression with LASSO, enhanced by cross-validation, subgroup analysis, and strategic handling of missing data. Each methodological decision, from variable selection to model validation, contributed to a nuanced understanding of the complex dynamics influencing wages, education, and experience. Moving forward, these insights offer a foundation for further research, policy-making, and targeted interventions designed to address the identified factors more effectively.
